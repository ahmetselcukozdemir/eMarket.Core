...
using eMarket.Core.Web.UI.Models.Admin;
using eMarket.Core.Web.UI.Models.Admin.Product;
using eMarket.Core.Web.UI.Models.Blog;
using eMarket.Core.Web.UI.Models.Category;
using eMarket.Core.Web.UI.Models.Member;
...
namespace eMarket.Web.UI.Controllers
{
    public class AdminController : Controller
    {
        ...
        public async Task<IActionResult> ProductImage(string productNumber, IEnumerable<IFormFile> files)
        {
            if (files.Count() > 0)
            {
                int a = 0;
                foreach (var formFile in files)
                {
                    ...
                    var checkHomeImage = _adminService.GetProductHomeImageCheck(data_id);
                    if (checkHomeImage == true)
                    {
